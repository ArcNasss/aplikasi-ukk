                    ...
                    <table class="table table-hover mb-0">
                        <thead style="background: #4f73df; color: white;">
                            <tr>
                                ...
                                <th class="border-0 py-3">Jumlah Buku</th>
                        ...
                        <tbody>
                           @forelse ($borrows as $index => $borrow)
                                ...
                                <td class="py-3 align-middle">{{$borrow->user->nomor_identitas}}</td>
                                <td class="py-3 align-middle">{{ $borrow->bookItem->book->judul ?? '' }}</td>
                                <td class="py-3 align-middle">{{$borrow->count()}}</td>
                                <td class="py-3 align-middle">{{ $borrow->created_at->format('d/m/Y') }}</td>
                                <td>
                                    <form action="{{route('peminjaman.approve', $borrow->id)}}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                      ...
                                      &nbsp;
                                      <form action="{{route('peminjaman.reject', $borrow->id)}}" method="POST">
                                        @csrf
                                        @method('PATCH')
